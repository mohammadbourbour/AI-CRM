...
<p align="center">
  <b>Inbound lead → AI qualification → CRM → Google Sheets → human approval → Telegram</b><br>
  n8n orchestrates the pipeline. FastAPI is the source of truth. Follow-up is not sent until a human approves.
</p>
...
| **Pre-qualify** | n8n **AI Agent** + official **Groq Chat Model** (`openai/gpt-oss-20b`) |
...
| **Approve** | Dashboard approval queue — then Telegram posts the decision |
...
Official n8n nodes — **AI Agent**, **Groq Chat Model**, **Telegram**, **Google Sheets** — plus HTTP for the FastAPI CRM.

<p align="center">
  <img src="docs/images/n8n-workflow.png" alt="n8n Lead Qualification workflow canvas" width="100%">
</p>

<p align="center"><sub>Color groups: intake → enrichment → AI Agent → CRM → Google Sheets → routing / HITL.</sub></p>
...
| **5. Routing & HITL** | IF hot → draft → Telegram alert | Customer copy is **not** sent here |
| **Approve** | Dashboard queue | `approve-followup` / `reject-followup` → Telegram decision post |
...
|-------|------|----------------|
| **n8n** | Visual flow, validation, AI Agent pre-qualify, Sheets upsert, Telegram sales alert | Stored priority, SQLite writes |
| **FastAPI** | CRM, deterministic scoring, HITL flag, Telegram on follow-up decisions | Canvas orchestration |
...
**AI that is honest**
- Groq (`openai/gpt-oss-20b`) for backend qualify/draft and n8n pre-qualify
...
- Telegram sales alert after qualification (hot and warm)
- Telegram post after approve / reject
- Google Sheets upsert when a spreadsheet id is configured
- Missing credentials → `skipped_unconfigured`, never marked as sent

**Ops**
- Docker Compose: API on `:8000`, n8n on `:5678`
- pytest on in-memory SQLite
- PII-masked logs (`j***@domain.com`)

## Potential

This is a **focused CRM automation slice**, not a multi-tenant product. The same pattern scales to a larger revenue team:
...
| Mock send + Telegram decision | CRM of record (HubSpot / Salesforce) + calendar booking |
| `GET /api/leads` unpaginated | Filters, search, owner assignment, SLA clocks |
| Dashboard approval queue | Inbox UI, Slack approve buttons |
...
2. Attach credentials: Groq, Telegram, Google Sheets
...
3. Open [http://localhost:8000](http://localhost:8000) and run **Invalid / Cold / Warm / Hot**

The dashboard posts the sample to `/webhook/lead-intake`. Watch the run under n8n **Executions**. Telegram and Google Sheets only send when configured; otherwise the status is `skipped_unconfigured`.
...
n8n public hooks (after activate): `POST /webhook/lead-intake`.
...
| Orchestration | n8n 2.x — AI Agent, Groq Chat Model, Telegram, Google Sheets, HTTP |
| LLM | Groq (`openai/gpt-oss-20b`), MockLLMProvider |
...
| Variable | Role |
|----------|------|
| `GROQ_API_KEY` / `GROQ_MODEL` | Backend LLM and n8n AI Agent. Default: `openai/gpt-oss-20b` |
| `TELEGRAM_BOT_TOKEN` / `TELEGRAM_CHAT_ID` | Sales-channel alerts and follow-up decisions |
...
- Follow-up CRM status is `mock_sent`; the approved draft is posted to Telegram for the sales team
